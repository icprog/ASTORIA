...
END =	5*360  # (segundos)-talvez modificar para calcular com START e END (5+3600)
...
STATION_SUBSERVER = 50  #Numero de Stations a cada SubServer (talvez fazer de outro jeito, ainda tem q ver)
PROTOCOL = 'Modbus'
	...
	for node in grid:
		if("branch" not in node.eid):
			...
			try:
				...
				world.connect(node,ns3_node,('P_from','P')) # decidir isso aqui (P_from)
...
def connect_producers(world, pvs, grid):#arrumar isso aqui
